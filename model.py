    ...
    def forward(self, x, src_padding_mask=None):
        ...
        if self.flash:
            ...
            if src_padding_mask is not None:
                att = att.masked_fill(src_padding_mask, float('-inf'))
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc    = nn.Linear(config.n_embd, config.dff, bias=config.bias)
        self.gelu    = nn.GELU()
        self.c_proj  = nn.Linear(config.dff, config.n_embd, bias=config.bias)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, src_padding_mask):         
        out1 = self.ln_1(x)
        out1 = x + self.attn(out1, src_padding_mask)

        out2 = self.ln_2(out1)
        out2 = out1 + self.ffn(out2)
        return out2
...
class Transformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.predict_head = nn.Sequential(
            ...
            nn.Linear(config.n_embd, 512, bias=False),
            nn.GELU(),
    ...
    def forward(self, x, targets=None):  
        ...
        logits = self.predict_head(x[:, 0, :])  # Chau: or mean or flatten the whole seq*embd???        
